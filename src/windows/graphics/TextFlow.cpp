...
HRESULT TextFlow::SetTextFormat(ComPtr<IDWriteTextFormat> textFormat)
{
    ...
    DWRITE_LINE_SPACING_METHOD line_space_method;
    FLOAT line_space, baseline;
    textFormat->GetLineSpacing(&line_space_method, &line_space, &baseline);
    if (line_space_method == DWRITE_LINE_SPACING_METHOD_DEFAULT) {
    ...
    } else if (line_space_method == DWRITE_LINE_SPACING_METHOD_UNIFORM) {
        flow_metrics_.line_height = line_space;
        flow_metrics_.baseline = baseline;
    } else {
        LOG(WARNING) << "Unsupported line spacing method " << (int)line_space_method;
    }
