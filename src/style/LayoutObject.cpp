...
void LayoutObject::reflow(FlowRoot fl, float viewport_width, absl::optional<float> viewport_height)
{
    ...
    bfc.contg_width = viewport_width;
    bfc.border_bottom_edge = bfc.margin_bottom_edge = 0.0f;
    bfc.contg_height = viewport_height;
...
void LayoutObject::paint(LayoutObject* o, graph2d::PaintContextInterface* painter)
{
    ...
    if (o->node && o->node->control_ && content_rect.has_value()) {
        painter->drawControl(scene2d::RectF::fromOriginSize(scene2d::PointF(), content_rect.value().size()),
                             o->node->control_.get());
    }
    ...
    if (o->flags & HAS_BLOCK_CHILD_FLAG) {
        ...
        if (o->ifc.has_value()) {
            rect = scene2d::RectF::fromOriginSize(scene2d::PointF(),
                                                  scene2d::DimensionF(o->ifc->getLayoutWidth(),
                                                                      o->ifc->getLayoutHeight()));
    ...
    if (depends_children_width) {
        if (o->flags & LayoutObject::HAS_BLOCK_CHILD_FLAG) {
            ...
            while (child) {
                ...
                float mbp_width = margin_left + border_left + padding_left + padding_right + border_right +
                    margin_right;
...
void LayoutObject::arrangeBlock(LayoutObject* o, BlockFormatContext& bfc, float viewport_width,
                                absl::optional<float> viewport_height)
{
    const Style& st = *o->style;
                                ...
                                ScrollbarPolicy scroll_y)
{
    ...
    if (st.display == DisplayType::Block) {
        ...
        if (o->flags & NEW_BFC_FLAG) {
            ...
            arrangeBfcChildren(o, inner_bfc, box, viewport_width, viewport_height);
            if (st.overflow_y == OverflowType::Visible) {
                ...
                bfc.border_bottom_edge = bfc.margin_bottom_edge = std::max(
                    bfc.margin_bottom_edge, box_content_bottom_edge);
            }
            ...
            arrangeBfcTop(o, bfc, box);
            arrangeBfcChildren(o, bfc, box, viewport_width, viewport_height);
            arrangeBfcBottom(o, bfc, box);
        }

        LayoutObject::arrangePositionedChildren(o, viewport_width, viewport_height);
    } else {
                                      ...
                                      float viewport_width, absl::optional<float> viewport_height)
{
    ...
    if (o->flags & HAS_BLOCK_CHILD_FLAG) {
        ...
        LayoutObject* child = o->first_child;
        while (child) {
            arrangeBlock(child, bfc, viewport_width, viewport_height);
            child = child->next_sibling;
        }
        ...
        o->ifc.emplace(o, bfc, content_rect.width());
        {
            ...
            LayoutObject* child = o->first_child;
            while (child) {
                prepare(child, *o->ifc, viewport_width, viewport_height);
                child = child->next_sibling;
            }
            o->ifc->arrangeX(o->style->text_align);
            child = o->first_child;
            while (child) {
                arrange(child, *o->ifc, viewport_width, viewport_height);
                child = child->next_sibling;
            }
            o->ifc->arrangeY();
        }
...
public:
    ...
    void addGlyphRun(style::LineBox* line, const scene2d::PointF& pos,
...
void LayoutObject::prepare(LayoutObject* o, InlineFormatContext& ifc, float viewport_width,
                           absl::optional<float> viewport_height)
{
    ...
    if (absl::holds_alternative<TextBox>(o->box)) {
        ...
        arrangeInlineBlock(o, ifc, viewport_width, viewport_height);
    } else if (o->flags & HAS_INLINE_CHILD_FLAG) {
        LayoutObject* child = o->first_child;
        while (child) {
            prepare(child, ifc, viewport_width, viewport_height);
            child = child->next_sibling;
        }
    }
}
...
void LayoutObject::arrange(LayoutObject* o, InlineFormatContext& ifc, float viewport_width,
                           absl::optional<float> viewport_height)
{
    const auto& st = *o->style;
    ...
    if (st.display == DisplayType::Inline) {
        if (o->flags & HAS_INLINE_CHILD_FLAG) {
            ...
            while (child) {
                arrange(child, ifc, viewport_width, viewport_height);
...
void LayoutObject::arrangeInlineBlock(LayoutObject* o, InlineFormatContext& ifc, float viewport_width,
                                      absl::optional<float> viewport_height)
{
    ...
    arrangeInlineBlockX(o, bfc, viewport_width, viewport_height, scroll_y);
    ...
    arrangeBfcChildren(o, inner_bfc, b, viewport_width, viewport_height);
...
void LayoutObject::arrangeInlineBlockChildren(LayoutObject* o,
                                              absl::optional<float> contg_height, float viewport_width,
                                              absl::optional<float> viewport_height)
{
...
void LayoutObject::arrangePositionedChildren(LayoutObject* o, float viewport_width,
                                             absl::optional<float> viewport_height)
{
    for (LayoutObject* po : o->positioned_children) {
        if (po->style->position == PositionType::Absolute
            || po->style->position == PositionType::Fixed) {
            reflow(FlowRoot{po, o}, viewport_width, viewport_height);
        } else {
...
static void split_up(LayoutObject* blk_contg_root, const std::deque<LayoutObject*>& inline_parent,
...
void LayoutTreeBuilder::prepareChild(scene2d::Node* node)
{
    if (node->type() == scene2d::NodeType::NODE_TEXT) {
        ...
        if (st.display == style::DisplayType::Block) {
            if (contg_->style->display == style::DisplayType::Block || contg_->style->display ==
                style::DisplayType::InlineBlock) {
