...
namespace
{
    ...
    using BoxShadowDeclarations = std::vector<BoxShadowSpec>;
    ...
    /**
     * box-shadow := 'inset'? color? (offset | offset_x offset_y)? blur_radius? (spread | spread_x spread_y)?
     */
    IResult<BoxShadowSpec> box_shadow_item(absl::string_view input)
    {
        auto res = separated_list1(spaces, value_spec)(input);
        if (!res.ok())
            return res.status();
        ...
        // Extract 'inset', color, and length values
        for (auto& spec : list) {
            if (!spec.value.has_value())
                continue;

            auto& v = spec.value.value();
            if (v.unit == ValueUnit::Keyword) {
                if (v.isKeyword("inset")) {
                    decl.inset = true;
                } else if (v.isKeyword("none")) {
                    decl.type = BoxShadowSpecType::None;
                } else if (v.isKeyword("inherit")) {
                    decl.type = BoxShadowSpecType::Inherit;
                } else if (v.isKeyword("initial")) {
                    decl.type = BoxShadowSpecType::Initial;
                } else {
                    if (named_color::fromString(v.keyword_val.c_str()).has_value()) {
                        decl.color = v;
                    } else {
                        return absl::InvalidArgumentError(input);
                    }
                }
            } else if (v.unit == ValueUnit::HexColor) {
                decl.color = v;
            } else if (v.unit == ValueUnit::Raw || v.unit == ValueUnit::Pixel
                || v.unit == ValueUnit::Point || v.unit == ValueUnit::Percent) {
                lengths.push_back(v);
            } else {
                return absl::InvalidArgumentError(input);
            }
        ...
        if (lengths.size() == 1) {
            ...
            decl.offset_y = lengths[1];
            decl.blur_radius = lengths[2];
            decl.spread_x = lengths[3];
            decl.spread_y = lengths[4];
        }
        return std::make_tuple(output, decl);
    }
    ...
    IResult<SingleDeclaration> box_shadow_decl(absl::string_view input)
    {
        ...
        auto [output, decls] = res1.value();
        SingleDeclaration sd;
        sd.name = base::string_intern("box-shadow");
        sd.value.type = ValueSpecType::Specified;
        sd.value.value = Value::fromBoxShadow(std::move(decls));
        return std::make_tuple(output, sd);
    }
    ...
    IResult<Declaration> declaration(absl::string_view input)
    {
        ...
        } else if (prop_name == base::string_intern("box-shadow")) {
            decl = box_shadow_decl(input1);
        } else {
